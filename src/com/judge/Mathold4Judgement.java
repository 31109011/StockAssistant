...
public class Mathold4Judgement extends IJudge{

    static int num=6;

    public Mathold4Judgement(KGraph sb) {
        super(sb, num);
    }
    ...
    public boolean hasFeature() {
        ...
        Candle c4=lstcandle.get(4);
        Candle c5=lstcandle.get(4);//当前
        if(isWrite(c0)==false){
            return false;
        }
        if((isWrite(c1)==true)||(isWrite(c2)==true)||
                (isWrite(c3)==true)||(isWrite(c4)==true)){
            return false;
        }
        //最后一根蜡烛线是白色的，
        if(isWrite(c5)==false){
        ...
        if((c4.higest<c5.open)||(c5.close>c4.higest)){
            return true;
        }else{
            return false;
        }
    ...
    public String judgeName() {
        return stockCode+stockName+":铺垫形态4";
    }
